...
sheets = ["C3 9.5 V", "C3 9 V", "C3 8.5 V", "C3 8 V", "C3 7.5 V", "C3 7 V", "C3 6.5 V", "C3 6 V", "C3 5.5 V", "C3 5 V", "C3 4.5 V"]
#sheets = ["C2 10 V"]
...
if True:
    ...
    while m < len(sheets):
        ...
        error = ((FWHM - half_max_left)/30)*10**-6
        time1.append(tM)
        FWHM = FWHM*10**-6
        LHS = max_V * FWHM * Vs[m]
        log_value1.append(np.log(LHS))
        log_value_error1.append(np.log(LHS)*(error/FWHM)*0.25)
        m += 1
    ...
    while m < len(sheets):
        ...
        FWHM = FWHM*10**-6
        LHS = max_V * FWHM * Vs[m]
        log_value2.append(np.log(LHS))
        log_value_error2.append(np.log(LHS)*(error/FWHM)*0.25)
        m += 1
...
a = plt.errorbar(time1, log_value1, yerr=log_value_error1,fmt='b.', lw=1.2, capsize=3, capthick=1.0)
plt.errorbar(time2, log_value2, yerr=log_value_error2,fmt='r.', lw=1.2, capsize=3, capthick=1.0)

#plt.savefig('Report/Graph.png') # Save the graph for a higher quality image

fit_par,cov = np.polyfit(time1, log_value1, 1, cov=True)
...
x_range_lower = 2.2
x_range_higher = 3.4       # Set x range limits here to adjust the plot with them and for the best fit line stuff below.
...
plt.plot(x_points, y_points, 'b-', linewidth=2.0)

fit_par,cov = np.polyfit(time2, log_value2, 1, cov=True)
...
print(f"Intercept is {intercept:.5f} ± {intercept_SE:.5f}")
x_range_lower = 2.3
x_range_higher = 3.7       # Set x range limits here to adjust the plot with them and for the best fit line stuff below.
...
plt.plot(x_points, y_points, 'r-', linewidth=2.0)
